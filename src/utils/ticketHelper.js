...
function getTicketTranscriptLabel(ticketChannel, ticket = null) {
  const channelName = ticketChannel?.name
    || ticket?.channelName
    || (ticket?.channelId ? ticketChannel?.guild?.channels?.cache?.get(ticket.channelId)?.name : null)
    || 'ticket';

  return String(channelName).replace(/^#/, '').trim() || 'ticket';
}
...
async function buildTicketTranscript(ticketChannel) {
  if (!ticketChannel || !ticketChannel.isTextBased()) {
    return null;
  }

  const filename = `${toSafeChannelName(ticketChannel.name || 'ticket')}-transcript.html`;
  return trans.createTranscript(ticketChannel, {
    filename,
    saveImages: true,
    poweredBy: false
  });
...
async function sendTicketTranscript(ticketChannel, ticket, guild, options = {}) {
  const transcript = await buildTicketTranscript(ticketChannel);
  if (!transcript) {
    return null;
  }

  const settings = options.settings || (guild ? await getTicketSettings(guild.id) : null);
  const transcriptTarget = options.transcriptChannel
    || (settings?.transcriptChannelId
      ? guild?.channels?.cache?.get(settings.transcriptChannelId) ?? await guild?.channels?.fetch(settings.transcriptChannelId).catch(() => null)
      : null)
    || ticketChannel;
  ...
  const transcriptLabel = getTicketTranscriptLabel(ticketChannel, ticket);
  let generatedByLabel = null;
  if (options.generatedBy) {
    generatedByLabel = guild?.members?.cache?.get(options.generatedBy)?.user?.username || null;
    if (!generatedByLabel && guild?.members?.fetch) {
      const member = await guild.members.fetch(options.generatedBy).catch(() => null);
      generatedByLabel = member?.user?.username || null;
    }
    if (!generatedByLabel) {
      generatedByLabel = options.generatedBy;
    }
  }
  await transcriptTarget.send({
    content: `Ticket transcript for ${transcriptLabel}${generatedByLabel ? ` generated by ${generatedByLabel}` : ''}`,
    files: [transcript]
  }).catch(() => null);
...
async function closeTicket(interaction, options = {}) {
  ...
  await sendTicketTranscript(channel, ticket.toObject(), interaction.guild, {
    settings,
